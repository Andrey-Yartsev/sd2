...
class CtrlSdRender extends CtrlCommon {
  ...
  static function renderStitic($bannerId) {
    ...
    system('phantomjs '.SD_PATH.'/phantomjs/genStatic.js '.PROJECT_KEY.' '.SITE_DOMAIN.' '.$bannerId.' '.Config::getVar('sd/renderKey').' '.WEBROOT_PATH);
  ...
  function action_ajax_animated() {
    ...
    system('phantomjs '.$sdPath.'/phantomjs/genAnimated.js '. //
      PROJECT_KEY.' '.SITE_DOMAIN.' '.$this->bannerId.' '.$framesCount.' '.Config::getVar('sd/renderKey').' '.WEBROOT_PATH);
